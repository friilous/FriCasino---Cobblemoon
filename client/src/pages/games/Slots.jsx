...
function makeStrip(finals) { const s=Array.from({length:STRIP_LEN},randSymId); s[STRIP_LEN-3]=finals[0];s[STRIP_LEN-2]=finals[1];s[STRIP_LEN-1]=finals[2]; return s }
...
function Reel({ reelIndex, spinTrigger, stopDelay, onStopped, isWin, getPendingSyms }) {
  ...
  useEffect(()=>{
    ...
    const finals=getPendingSyms(reelIndex),newStrip=makeStrip(finals)
    applyPos(0);setStrip(newStrip)
    const total=stopDelay+550,t0=performance.now()
    const animate=(now)=>{
      const p=Math.min((now-t0)/total,1),e=1-Math.pow(1-p,3)
      applyPos(e*endPx)
      if(p<1){rafRef.current=requestAnimationFrame(animate)}
      else{applyPos(endPx);stopRef.current=setTimeout(()=>{setSettled(true);setFlash(true);setTimeout(()=>setFlash(false),600);onStopped?.()},20)}
    }
    rafRef.current=requestAnimationFrame(()=>{applyPos(0);rafRef.current=requestAnimationFrame(animate)})
    return()=>{cancelAnimationFrame(rafRef.current);clearTimeout(stopRef.current)}
  },[spinTrigger])
  const midWins=settled&&isWin
  const IDX_TOP=STRIP_LEN-3,IDX_MID=STRIP_LEN-2,IDX_BOT=STRIP_LEN-1
  return (
    <div style={{width:100,height:SLOT_H*3,background:'#080814',borderRadius:12,
      border:`2px solid ${midWins?C.gold:flash?'#ffffff30':C.border}`,
      boxShadow:midWins?`0 0 24px ${C.gold}40`:'none',overflow:'hidden',position:'relative',
      transition:'border-color .3s,box-shadow .4s'}}>
      <div style={{position:'absolute',inset:0,zIndex:2,pointerEvents:'none',
        background:'linear-gradient(to bottom,#08081490 0%,transparent 22%,transparent 78%,#08081490 100%)'}}/>
      <div style={{position:'absolute',left:0,right:0,top:SLOT_H,height:SLOT_H,zIndex:1,
        borderTop:`1px solid ${midWins?C.gold+'80':C.gold+'20'}`,
        borderBottom:`1px solid ${midWins?C.gold+'80':C.gold+'20'}`,
        background:midWins?C.gold+'10':'transparent',transition:'all .4s',pointerEvents:'none'}}/>
      <div ref={bandRef} style={{position:'absolute',top:0,left:0,right:0,height:STRIP_LEN*SLOT_H,
        transform:`translateY(${endPx}px)`,zIndex:3,willChange:'transform'}}>
        {strip.map((id,i)=>{
          const sym=symById(id),isMid=i===IDX_MID,isVis=i===IDX_TOP||i===IDX_MID||i===IDX_BOT
          const winSlot=settled&&isWin&&isMid
          const size=settled&&isMid?68:settled&&isVis?46:54
          const opacity=settled&&isVis&&!isMid?.35:1
          return (
            <div key={i} style={{height:SLOT_H,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',position:'relative'}}>
              <img src={sym.sprite} alt={sym.label} draggable={false} style={{
                width:size,height:size,imageRendering:'pixelated',objectFit:'contain',opacity,
                filter:winSlot?`drop-shadow(0 0 12px ${sym.color}) brightness(1.3)`:settled&&isMid?sym.isWild?`drop-shadow(0 0 8px ${sym.color})`:'brightness(1.05)':'brightness(0.8)',
                transition:settled?'filter .3s,opacity .2s,width .15s,height .15s':'none',userSelect:'none'}}/>
              {settled&&isMid&&<div style={{position:'absolute',bottom:3,fontSize:7,fontWeight:700,
                color:winSlot?sym.color:C.muted,textTransform:'uppercase',letterSpacing:.5}}>
                {sym.isWild?'✦ WILD ✦':sym.label}</div>}
            </div>)
        })}
      </div>
    </div>
...
export default function Slots() {
  ...
  useEffect(()=>{
    if(stoppedCount===3&&pendingResult.current){
      ...
      updateBalance(r.balance);setResult(r);setSpinning(false)
  ...
  function handleReelStopped(i){
    setReelStopped(p=>{const n=[...p];n[i]=true;return n})
    ...
    <div style={{minHeight:'100vh',background:C.bg,padding:'14px',boxSizing:'border-box'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'260px 1fr',gap:12,alignItems:'start'}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:10}}>
          <div style={{background:C.surf1,border:`1px solid ${C.border}`,borderRadius:12,padding:14}}>
            ...
            <button onClick={handleSpin} disabled={spinning||bet<10||bet>(user?.balance??0)}
              style={{width:'100%',marginTop:12,padding:'13px',
                background:spinning?C.dim:C.gold,color:spinning?C.muted:'#06060f',
          ...
          {result&&!spinning&&(
            <div style={{background:isWin?`${C.green}10`:`${C.red}08`,
              border:`1px solid ${isWin?C.green+'30':C.red+'20'}`,borderRadius:12,padding:14,textAlign:'center'}}>
              {isJackpot&&<div style={{color:C.gold,fontWeight:900,fontSize:13,marginBottom:6,letterSpacing:2}}>🏆 JACKPOT !</div>}
              {isWin?(
                <>
                  {winSym&&<div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8,justifyContent:'center'}}>
                    <img src={winSym.sprite} alt={winSym.label} style={{width:36,height:36,imageRendering:'pixelated',filter:`drop-shadow(0 0 8px ${winSym.color})`}}/>
                    <div style={{textAlign:'left'}}>
                      <div style={{color:winSym.color,fontWeight:800,fontSize:13}}>{winSym.label}{result.hasWild&&<span style={{color:C.gold,marginLeft:4,fontSize:10}}>✨ Wild</span>}</div>
                ...
                <>
                  ...
                  <div style={{color:C.red,fontSize:16,fontWeight:800,marginTop:4}}>−{bet.toLocaleString()}</div>
                </>
              )}
            </div>
          )}
          ...
          {history.length>0&&(
            <div style={{display:'flex',gap:5,flexWrap:'wrap'}}>
              {history.map(h=>(
                <div key={h.id} style={{fontSize:10,fontWeight:700,padding:'3px 8px',borderRadius:20,
                  background:`${C.green}12`,border:`1px solid ${C.green}30`,color:C.green}}>
                  +{h.payout.toLocaleString()} <span style={{opacity:.5}}>×{h.mult}</span>
                </div>
              ))}
            </div>
          )}
          ...
          {/* Règles */}
          <div style={{background:C.surf1,border:`1px solid ${C.border}`,borderRadius:12,padding:14}}>
            <div style={{fontSize:10,color:C.gold,fontWeight:700,textTransform:'uppercase',letterSpacing:1,marginBottom:8}}>📋 Gains</div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 44px 44px',gap:'3px 6px',alignItems:'center'}}>
              ...
              {PAYOUTS_TABLE.map(p=>{
                ...
                return[
                  <div key={p.id+'n'} style={{display:'flex',alignItems:'center',gap:5,padding:'2px 0'}}>
            ...
            <div style={{marginTop:8,paddingTop:6,borderTop:`1px solid ${C.border}`,fontSize:10,color:C.muted,lineHeight:1.8}}>
        ...
        <div style={{background:C.surf1,border:`2px solid ${isJackpot?C.gold:isWin?C.gold+'50':C.border}`,
          ...
          <div style={{textAlign:'center',marginBottom:16}}>
            ...
            <div style={{fontSize:11,color:C.muted,marginTop:2}}>Ligne centrale · 3 rouleaux · Wild disponible</div>
          </div>
          {isJackpot&&(
            <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:8,padding:'8px 16px',
              background:`${C.gold}12`,border:`1px solid ${C.gold}40`,borderRadius:10,marginBottom:14}}>
              <span style={{color:C.gold,fontWeight:900,fontSize:15,letterSpacing:3}}>🏆 JACKPOT 🏆</span>
            </div>
          )}
          <div style={{background:'#080814',borderRadius:16,border:`1px solid ${C.border}`,padding:'20px 12px 16px',position:'relative'}}>
            <div style={{position:'absolute',top:0,left:0,right:0,height:3,borderRadius:'16px 16px 0 0',
              background:isJackpot?`linear-gradient(90deg,transparent,${C.gold},transparent)`:isWin?`linear-gradient(90deg,transparent,${C.gold}60,transparent)`:`linear-gradient(90deg,transparent,${C.border},transparent)`,
              transition:'background .5s'}}/>
            <div style={{display:'flex',gap:10,justifyContent:'center'}}>
              {[0,1,2].map(i=>(
                <Reel key={i} reelIndex={i} spinTrigger={spinTrigger} stopDelay={900+i*650}
                  onStopped={()=>handleReelStopped(i)} isWin={isWin} getPendingSyms={getPendingSyms}/>
              ))}
            </div>
            {spinning&&(
              <div style={{display:'flex',justifyContent:'center',gap:10,marginTop:12}}>
                {[0,1,2].map(i=>(
                  <div key={i} style={{width:100,height:2,borderRadius:1,
                    background:reelStopped[i]?C.gold:C.dim,
                    boxShadow:reelStopped[i]?`0 0 6px ${C.gold}`:'none',
