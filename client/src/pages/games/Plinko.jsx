...
const CW=540, CH=520, ROWS=8, BUCKETS=9, GAP=46, PEG_R=6, BALL_R=9
...
function getPegPos(row,col){
  const count=row+2,totalW=(count-1)*GAP,startX=CW/2-totalW/2
...
function drawPeg(ctx,x,y,hitT){
  ...
  gr.addColorStop(0,delta<300?'#ffe066':'#9090cc');gr.addColorStop(.5,delta<300?'#f0b429':'#5050aa');gr.addColorStop(1,'#1e1e38')
  ctx.beginPath();ctx.arc(x,y,PEG_R,0,Math.PI*2);ctx.fillStyle=gr;ctx.fill()
...
function drawBucket(ctx,i,mults,active){
  ...
  ctx.beginPath()
  ctx.moveTo(rx+rr,by);ctx.lineTo(rx+bw-rr,by);ctx.arcTo(rx+bw,by,rx+bw,by+rr,rr);ctx.lineTo(rx+bw,by+BUCKET_H-rr)
  ctx.arcTo(rx+bw,by+BUCKET_H,rx+bw-rr,by+BUCKET_H,rr);ctx.lineTo(rx+rr,by+BUCKET_H)
  ctx.arcTo(rx,by+BUCKET_H,rx,by+BUCKET_H-rr,rr);ctx.lineTo(rx,by+rr);ctx.arcTo(rx,by,rx+rr,by,rr);ctx.closePath()
  ...
  ctx.fillStyle=active?'#000':'#fff';ctx.font=`bold ${mult>=10?8:10}px 'Courier New',monospace`
  ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(`${mult}×`,bx,by+BUCKET_H*.38)
  ...
  for(let s=0;s<steps;s++){
    ...
    for(let row=0;row<ROWS;row++)for(let col=0;col<=row+1;col++){
      ...
      if(dist<minD&&dist>.01){
        ...
        if(dot<0){ball.vx-=(1+RESTITUTION)*dot*nx;ball.vy-=(1+RESTITUTION)*dot*ny;ball.vx+=(Math.random()-.5)*.8}
        const key=`${row}-${col}`;if(!pegHits[key]||Date.now()-pegHits[key]>200)pegHits[key]=Date.now()
      }
    }
...
function usePlinkoEngine(canvasRef,risk,onBallLanded){
  ...
  useEffect(()=>{
    const loop=(now)=>{
      ...
      if(canvas){
        ...
        ctx.fillStyle='rgba(240,180,41,0.45)';ctx.beginPath();ctx.moveTo(CW/2,5);ctx.lineTo(CW/2-5,15);ctx.lineTo(CW/2+5,15);ctx.closePath();ctx.fill()
        for(let row=0;row<ROWS;row++)for(let col=0;col<=row+1;col++){const{x,y}=getPegPos(row,col);drawPeg(ctx,x,y,pegHitsRef.current[`${row}-${col}`])}
  ...
  const dropBall=useCallback((id,targetBucket,color)=>{
    const startX=CW/2+(Math.random()-.5)*4
    ballsRef.current=[...ballsRef.current,{id,x:startX,y:10,vx:(Math.random()-.5)*.5,vy:2,angle:0,angleV:0,color,targetBucket,done:false,hitBucket:undefined}]
  },[])
  return{dropBall}
}
...
export default function Plinko() {
  ...
  const handleBallLanded=useCallback((ballId)=>{
    ...
    setStats(p=>({total:p.total+1,wins:p.wins+(isWin?1:0),pnl:p.pnl+pnl}))
    setHistory(p=>[{id:ballId,mult,pnl,color},...p].slice(0,14))
    setDropping(false)
  },[])
        ...
        <div style={{display:'flex',flexDirection:'column',gap:10}}>
          <div style={{background:C.surf1,border:`1px solid ${C.border}`,borderRadius:12,padding:14}}>
                ...
                {Object.entries(RISK_LABELS).map(([id,info])=>(
                  <button key={id} onClick={()=>setRisk(id)} disabled={dropping}
                    style={{flex:1,padding:'8px 0',borderRadius:8,fontSize:12,fontWeight:700,cursor:'pointer',
                      ...
                      color:risk===id?info.color:C.muted,transition:'all .15s'}}>
                    {info.label}
                  </button>
                ))}
              </div>
            ...
            <button onClick={handleDrop} disabled={dropping||bet<10||bet>(user?.balance??0)}
              style={{width:'100%',marginTop:12,padding:'13px',
                background:dropping?C.dim:C.gold,color:dropping?C.muted:'#06060f',
                fontWeight:800,fontSize:15,borderRadius:10,border:'none',
                cursor:dropping?'not-allowed':'pointer',
                opacity:bet>(user?.balance??0)?.5:1,
                boxShadow:dropping?'none':`0 0 20px ${C.gold}44`,transition:'all .15s'}}>
              {dropping?<span style={{display:'flex',alignItems:'center',justifyContent:'center',gap:8}}>
                <span style={{width:14,height:14,borderRadius:'50%',border:'2px solid #f0b42940',borderTopColor:C.gold,display:'inline-block',animation:'spin .7s linear infinite'}}/>
                La bille tombe...
              </span>:'⚪ Lâcher la bille'}
            </button>
          </div>

          {/* Dernier résultat */}
          {lastResult&&(
            <div style={{background:lastResult.isWin?`${C.green}10`:`${C.red}08`,
              border:`1px solid ${lastResult.isWin?C.green+'30':C.red+'20'}`,borderRadius:12,padding:14,textAlign:'center'}}>
              <div style={{fontSize:11,color:C.muted,marginBottom:2}}>Dernier lancer · ×{lastResult.mult}</div>
              <div style={{fontSize:22,fontWeight:900,color:lastResult.isWin?C.green:C.muted}}>
          ...
          {stats.total>0&&(
            <div style={{background:C.surf1,border:`1px solid ${C.border}`,borderRadius:12,padding:14}}>
              ...
              {[['Lancers',stats.total],['Victoires',`${stats.wins} (${Math.round(stats.wins/stats.total*100)}%)`],['P&L',`${stats.pnl>=0?'+':''}${stats.pnl.toLocaleString()}`]].map(([l,v])=>(
                <div key={l} style={{display:'flex',justifyContent:'space-between',fontSize:12,padding:'3px 0'}}>
                  <span style={{color:C.muted}}>{l}</span>
                  <span style={{fontWeight:700,color:l==='P&L'?(stats.pnl>=0?C.green:C.red):C.txt}}>{v}</span>
                </div>
              ))}
            </div>
          )}

          {/* Règles / multiplicateurs */}
          <div style={{background:C.surf1,border:`1px solid ${C.border}`,borderRadius:12,padding:14}}>
            <div style={{fontSize:10,color:C.gold,fontWeight:700,textTransform:'uppercase',letterSpacing:1,marginBottom:10}}>📋 Multiplicateurs</div>
            {Object.entries(RISK_LABELS).map(([r,info])=>(
              <div key={r} style={{marginBottom:8,padding:'8px',borderRadius:8,
                background:r===risk?C.surf2:'transparent',
                border:`1px solid ${r===risk?C.border:'transparent'}`,
                opacity:r===risk?1:.4,transition:'all .2s'}}>
                <div style={{fontSize:11,fontWeight:700,color:info.color,marginBottom:5}}>{info.label}</div>
                <div style={{display:'flex',gap:3,flexWrap:'wrap'}}>
                  {MULTIPLIERS[r].map((m,i)=>(
                    <div key={i} style={{fontSize:9,fontWeight:700,padding:'2px 5px',borderRadius:4,fontFamily:'monospace',
                      background:BUCKET_COLORS[i]+'25',border:`1px solid ${BUCKET_COLORS[i]}50`,color:'#fff'}}>
                      {m}×
                    </div>
                  ))}
                </div>
            ...
            <div style={{marginTop:6,fontSize:10,color:C.muted,lineHeight:1.7}}>
              <div>🎯 La bille tombe à travers les picots</div>
              <div>🏆 Risque élevé = jackpot ×37.5 possible</div>
              <div>📊 Centre = fréquent · Bords = rare mais gros</div>
            </div>
          </div>
        ...
        <div style={{background:C.surf1,border:`1px solid ${C.border}`,borderRadius:16,padding:16}}>
          <div style={{textAlign:'center',marginBottom:14}}>
            ...
            <div style={{fontSize:11,color:C.muted,marginTop:2}}>
              Risque : <span style={{color:riskInfo.color,fontWeight:700}}>{riskInfo.label}</span>
            ...
            <div style={{display:'flex',gap:5,flexWrap:'wrap'}}>
              {history.map(h=>(
                <div key={h.id} style={{fontSize:10,fontWeight:700,padding:'3px 8px',borderRadius:20,
                  background:h.pnl>=0?`${C.green}12`:`${C.dim}`,
                  border:`1px solid ${h.color}35`,color:h.pnl>=0?C.green:C.muted}}>
                  {h.pnl>=0?'+':''}{h.pnl.toLocaleString()} <span style={{opacity:.5}}>×{h.mult}</span>
                </div>
              ))}
            </div>
          )}
