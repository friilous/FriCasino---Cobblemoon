...
// Canvas fixe — pas trop grand
const CW=480,CH=420,ROWS=8,BUCKETS=9,GAP=42,PEG_R=5,BALL_R=8
const BUCKET_H=36,FIRST_Y=60,BUCKET_Y=FIRST_Y+(ROWS-1)*GAP+GAP+4
const GRAVITY=.32,RESTITUTION=.38,FRICTION=.993
...
const BTYPES=['Dragon','Psy','Électrik','Feu','Normal','Feu','Électrik','Psy','Dragon']
...
function pegPos(row,col){const n=row+2,tw=(n-1)*GAP,sx=CW/2-tw/2;return{x:sx+col*GAP,y:FIRST_Y+row*GAP}}
...
function dpeg(ctx,x,y,ht){
  ...
  if(d<800){const p=d/800,ex=(1-Math.abs(p*2-1))*12;ctx.beginPath();ctx.arc(x,y,PEG_R+ex,0,Math.PI*2);ctx.fillStyle=`rgba(255,220,100,${.25*(1-p)})`;ctx.fill()}
  ...
  ctx.beginPath();ctx.arc(x-1.5,y-1.5,2,0,Math.PI*2);ctx.fillStyle='rgba(255,255,255,0.3)';ctx.fill()
}
function dball(ctx,x,y,col,angle){
  ...
  ctx.shadowColor=col;ctx.shadowBlur=12;ctx.save();ctx.translate(x,y);ctx.rotate(angle)
  ...
  ctx.restore();ctx.shadowBlur=0;ctx.beginPath();ctx.arc(x-2.5,y-2.5,2,0,Math.PI*2);ctx.fillStyle='rgba(255,255,255,0.4)';ctx.fill()
}
function dbucket(ctx,i,mults,active){
  const bx=bkX(i),by=BUCKET_Y,bw=GAP-3,col=BCOLS[i],mult=mults[i]
  if(active){ctx.shadowColor=col;ctx.shadowBlur=24}
  const rx=bx-bw/2,rr=6
  ...
  ctx.fillStyle=active?col:col+'28';ctx.fill();ctx.strokeStyle=col;ctx.lineWidth=active?2:1;ctx.stroke();ctx.shadowBlur=0
  ctx.fillStyle=active?'#000':'#fff';ctx.font=`bold ${mult>=10?8:9}px monospace`;ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(`${mult}×`,bx,by+BUCKET_H*.38)
}
function updateBall(ball,hits,dt){
  ...
  for(let s=0;s<st;s++){
    ...
    const sp=Math.hypot(ball.vx,ball.vy);if(sp>16){ball.vx=ball.vx/sp*16;ball.vy=ball.vy/sp*16}
    ...
    for(let r=0;r<ROWS;r++)for(let c=0;c<=r+1;c++){
      const{x:px,y:py}=pegPos(r,c),dx=ball.x-px,dy=ball.y-py,dist=Math.hypot(dx,dy),md=PEG_R+BALL_R
      if(dist<md&&dist>.01){const nx=dx/dist,ny=dy/dist,ov=md-dist;ball.x+=nx*ov;ball.y+=ny*ov;const dot=ball.vx*nx+ball.vy*ny;if(dot<0){ball.vx-=(1+RESTITUTION)*dot*nx;ball.vy-=(1+RESTITUTION)*dot*ny;ball.vx+=(Math.random()-.5)*.7};const k=`${r}-${c}`;if(!hits[k]||Date.now()-hits[k]>200)hits[k]=Date.now()}
    }
...
function useEngine(cvs,risk,onLand){
  ...
  useEffect(()=>{
    const loop=now=>{
      ...
      for(const b of balls.current){if(!b.done){updateBall(b,hits.current,dt);if(b.done&&b.hitBucket!==undefined){flash.current={bkt:b.hitBucket,t:Date.now()};onLand(b.id,b.hitBucket);setTimeout(()=>{balls.current=balls.current.filter(x=>x.id!==b.id)},600)}}}
      const c=cvs.current
      if(c){
        const ctx=c.getContext('2d'),ml=MULTS[rRef.current],fl=flash.current
        ctx.fillStyle='#05050e';ctx.fillRect(0,0,CW,CH)
        ctx.setLineDash([3,5]);ctx.strokeStyle='rgba(240,180,41,0.2)';ctx.lineWidth=1.2;ctx.beginPath();ctx.moveTo(CW/2,3);ctx.lineTo(CW/2,FIRST_Y-16);ctx.stroke();ctx.setLineDash([])
        ctx.fillStyle='rgba(240,180,41,0.5)';ctx.beginPath();ctx.moveTo(CW/2,4);ctx.lineTo(CW/2-5,13);ctx.lineTo(CW/2+5,13);ctx.closePath();ctx.fill()
        for(let r=0;r<ROWS;r++)for(let c2=0;c2<=r+1;c2++){const{x,y}=pegPos(r,c2);dpeg(ctx,x,y,hits.current[`${r}-${c2}`])}
  ...
  const drop=useCallback((id,tb,col)=>{const sx=CW/2+(Math.random()-.5)*4;balls.current=[...balls.current,{id,x:sx,y:10,vx:(Math.random()-.5)*.5,vy:1.8,angle:0,color:col,done:false,hitBucket:undefined}]},[])
  return{drop}
}
let uid=0
...
export default function Plinko(){
  ...
  const onLand=useCallback(id=>{
    ...
    const{payout,mult,pnl,win,color}=p
    setLast({payout,mult,pnl,win,color});setHistory(h=>[{id,mult,pnl,color},...h].slice(0,12))
    setStats(s=>({n:s.n+1,w:s.w+(win?1:0),pnl:s.pnl+pnl}));setDropping(false)
  },[])
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 280px',gap:12,alignItems:'start'}}>
        ...
        <div style={{background:C.surf,border:`1px solid ${C.border}`,borderRadius:18,padding:20,display:'flex',flexDirection:'column',alignItems:'center',gap:14}}>
          ...
          <div style={{background:'#05050e',borderRadius:14,border:`1px solid ${C.border}`,overflow:'hidden'}}>
            <canvas ref={cvs} width={CW} height={CH} style={{width:CW,height:CH,display:'block'}}/>
          </div>

          {/* Dernier résultat */}
          {last&&(
            <div style={{padding:'10px 20px',background:last.win?`${C.green}0e`:`${C.red}08`,border:`1px solid ${last.win?C.green+'28':C.red+'18'}`,borderRadius:12,display:'flex',alignItems:'center',gap:16}}>
              <div style={{fontSize:13,color:C.muted}}>×{last.mult}</div>
              <div style={{fontSize:22,fontWeight:900,color:last.win?C.green:C.muted}}>{last.pnl>=0?'+':''}{last.pnl.toLocaleString()}<span style={{fontSize:12,fontWeight:400,marginLeft:4}}>jetons</span></div>
            </div>
          )}
          ...
          {history.length>0&&(
            <div style={{display:'flex',gap:5,flexWrap:'wrap',justifyContent:'center'}}>
              {history.map(h=>(
                <div key={h.id} style={{fontSize:11,fontWeight:700,padding:'3px 9px',borderRadius:20,background:h.pnl>=0?`${C.green}12`:`${C.red}08`,border:`1px solid ${h.color}35`,color:h.pnl>=0?C.green:C.muted}}>
                  {h.pnl>=0?'+':''}{h.pnl.toLocaleString()} <span style={{opacity:.5,fontSize:10}}>×{h.mult}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{display:'flex',flexDirection:'column',gap:10}}>
          ...
          {stats.n>0&&(
            <div style={{background:C.surf,border:`1px solid ${C.border}`,borderRadius:14,padding:14}}>
              <div style={{fontSize:11,fontWeight:700,color:C.muted,textTransform:'uppercase',letterSpacing:1,marginBottom:8}}>Session</div>
              {[['Lancers',stats.n,C.txt],['Victoires',`${stats.w}/${stats.n}`,C.txt],['P&L',`${stats.pnl>=0?'+':''}${stats.pnl.toLocaleString()}`,stats.pnl>=0?C.green:C.red]].map(([l,v,col])=>(
                <div key={l} style={{display:'flex',justifyContent:'space-between',fontSize:12,padding:'4px 0'}}><span style={{color:C.muted}}>{l}</span><span style={{fontWeight:700,color:col}}>{v}</span></div>
              ))}
            </div>
          )}
          ...
          {/* Règles */}
          <div style={{background:C.surf,border:`1px solid ${C.border}`,borderRadius:14,padding:14}}>
            <div style={{fontSize:11,fontWeight:700,color:C.gold,textTransform:'uppercase',letterSpacing:1,marginBottom:10}}>Multiplicateurs</div>
            {Object.entries(RISK).map(([r,info])=>(
              <div key={r} style={{marginBottom:8,opacity:r===risk?1:.35,transition:'opacity .2s'}}>
                <div style={{fontSize:10,fontWeight:700,color:info.color,marginBottom:4}}>{info.label}</div>
                <div style={{display:'flex',gap:3,flexWrap:'wrap'}}>
                  {MULTS[r].map((m,i)=>(
                    <div key={i} style={{fontSize:9,fontWeight:700,padding:'2px 5px',borderRadius:4,fontFamily:'monospace',background:BCOLS[i]+'28',border:`1px solid ${BCOLS[i]}50`,color:'#fff'}}>{m}×</div>
                  ))}
                </div>
              </div>
            ))}
            <div style={{marginTop:8,paddingTop:8,borderTop:`1px solid ${C.dim}`,fontSize:10,color:C.muted,lineHeight:1.8}}>
              ...
              <div>🏆 Élevé : jackpot ×37.5 possible</div>
            </div>
          </div>
        </div>
      </div>
