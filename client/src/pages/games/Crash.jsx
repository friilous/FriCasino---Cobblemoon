...
const MONS=[{dex:25,name:'Pikachu'},{dex:6,name:'Dracaufeu'},{dex:149,name:'Dracolosse'},{dex:131,name:'Lokhlass'},{dex:143,name:'Ronflex'},{dex:94,name:'Ectoplasma'}]
...
function draw(ctx,pts,mult,phase,crash,mon){
  ...
  ctx.beginPath();ctx.moveTo(pts[0].x,H);for(const p of pts)ctx.lineTo(p.x,p.y);ctx.lineTo(pts[pts.length-1].x,H);ctx.closePath();ctx.fillStyle=col+'15';ctx.fill()
  ...
  if(mon?.img&&pts.length){const l=pts[pts.length-1],sz=36;ctx.save();if(phase==='crashed'){ctx.globalAlpha=.4;ctx.filter='grayscale(1)'}ctx.drawImage(mon.img,l.x-sz/2,l.y-sz-4,sz,sz);ctx.restore()}
  ctx.font='bold 34px monospace';ctx.textAlign='left';ctx.textBaseline='top';ctx.fillStyle=col;ctx.shadowColor=col;ctx.shadowBlur=14
  ctx.fillText(phase==='crashed'?`💥 ${crash?.toFixed(2)}×`:`${mult.toFixed(2)}×`,14,12);ctx.shadowBlur=0
...
export default function Crash(){
  ...
  useEffect(()=>loadMon(),[])
  function toY(m){const W=cvs.current?.height||260;const max=Math.max(rRef.current?.crashPoint||2,mRef.current,2);return W-14-(Math.log(m)/Math.log(max+.5))*(W-28)}
  function toX(ms){const W=cvs.current?.width||600;return 14+Math.min((ms/Math.max(3000,(rRef.current?.crashPoint||2)*1200))*(W-28),W-14)}
        ...
        <div style={{width:220,flexShrink:0,display:'flex',flexDirection:'column',gap:10}}>
          <div style={{background:C.surf,border:`1px solid ${C.border}`,borderRadius:14,padding:16}}>
            ...
            <button onClick={play} disabled={waiting||(!running&&(bet<10||bet>(user?.balance||0)))}
              style={{width:'100%',marginTop:14,padding:'14px',background:waiting?C.dim:running?'#0c260c':C.gold,color:waiting?C.muted:running?C.green:'#06060f',fontWeight:800,fontSize:16,borderRadius:10,border:running?`1px solid ${C.green}40`:'none',cursor:waiting?'not-allowed':'pointer',transition:'all .15s',boxShadow:waiting?'none':running?`0 0 20px ${C.green}30`:`0 0 20px ${C.gold}44`}}>
              {waiting?'En attente…':running?'💰 Encaisser !':'🚀 Lancer'}
            </button>
            {running&&<div style={{marginTop:8,textAlign:'center',fontSize:11,color:C.muted}}>Maintenant : <span style={{color:C.green,fontWeight:700}}>{Math.floor(bet*mult).toLocaleString()} jetons</span></div>}
          </div>

          {result&&phase!=='running'&&(
            <div style={{background:result.isWin?`${C.green}0e`:`${C.red}08`,border:`1px solid ${result.isWin?C.green+'28':C.red+'18'}`,borderRadius:12,padding:14}}>
              <div style={{fontSize:13,fontWeight:700,color:result.isWin?C.green:C.red,marginBottom:4}}>{result.isWin?`✅ ×${result.at?.toFixed(2)}`:`💥 Crash ×${result.crashPoint?.toFixed(2)}`}</div>
          ...
          {history.length>0&&(
            <div style={{display:'flex',gap:5,flexWrap:'wrap'}}>
              {history.map((h,i)=><div key={i} style={{fontSize:10,fontWeight:700,padding:'3px 8px',borderRadius:16,background:h.win?`${C.green}12`:`${C.red}10`,border:`1px solid ${h.win?C.green+'28':C.red+'20'}`,color:h.win?C.green:C.red}}>×{h.mult.toFixed(2)}</div>)}
            </div>
          )}
        </div>

        {/* CENTRE — Canvas */}
        <div style={{flex:1,background:C.surf,border:`1px solid ${C.border}`,borderRadius:18,padding:20,display:'flex',flexDirection:'column',gap:14}}>
          <div style={{textAlign:'center'}}><div style={{fontSize:22,fontWeight:900,color:C.gold,letterSpacing:4}}>CRASH</div><div style={{fontSize:11,color:C.muted,marginTop:3}}>Encaisse avant l'explosion</div></div>
          <div style={{background:'#060610',borderRadius:14,border:`1px solid ${C.border}`,overflow:'hidden',position:'relative'}}>
            <canvas ref={cvs} width={600} height={260} style={{width:'100%',display:'block'}}/>
            {mon&&<div style={{position:'absolute',bottom:8,right:10,display:'flex',alignItems:'center',gap:5}}><img src={SPRITE(mon.dex)} style={{width:20,height:20,imageRendering:'pixelated',opacity:.4}} alt=""/><span style={{fontSize:9,color:C.muted}}>{mon.name}</span></div>}
          </div>
        </div>

        {/* DROITE — Règles */}
        <div style={{width:220,flexShrink:0,background:C.surf,border:`1px solid ${C.border}`,borderRadius:14,padding:14}}>
          <div style={{fontSize:11,fontWeight:700,color:C.gold,textTransform:'uppercase',letterSpacing:1,marginBottom:12}}>Comment jouer</div>
          {[{icon:'🚀',t:'Lance',d:'le ×1 commence à monter'},{icon:'💰',t:'Encaisse',d:'avant le crash pour gagner'},{icon:'💥',t:'Crash avant toi',d:'= tout est perdu'},{icon:'🤖',t:'Auto-cashout',d:'encaisse à ton × automatique'}].map(({icon,t,d})=>(
          ...
          <div style={{marginTop:12,fontSize:10,color:C.muted,lineHeight:1.9}}>
