...
export default function Classement() {
  ...
  const [tab,     setTab]     = useState('topBestWin')
    ...
    <div style={{ minHeight:'100vh', background:C.bg, padding:'24px 28px', boxSizing:'border-box' }}>
      ...
      {loading ? (
        ...
        <div style={{ textAlign:'center', padding:'60px 0' }}>
          ...
          <button onClick={load} style={{ fontSize:11, color:C.gold, background:'none', border:`1px solid ${C.gold}40`, borderRadius:8, padding:'6px 14px', cursor:'pointer' }}>
            Réessayer
          </button>
        </div>
      ) : (
        <div style={{ background:C.surf, border:`1px solid ${C.border}`, borderRadius:14, overflow:'hidden' }}>
          {rows.length === 0 ? (
            <div style={{ textAlign:'center', color:C.muted, padding:'40px 0', fontSize:12 }}>
              Aucune donnée — sois le premier à jouer !
            </div>
          ) : (
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
              ...
              <tbody>
                {rows.map((row, i) => (
                  <tr key={i}
                    style={{ borderTop:`1px solid ${C.dim}`, background: i===0?`${C.gold}05`:i===1?`${C.muted}04`:'transparent', transition:'background .15s' }}
...
function getColumns(tab) {
  ...
  switch (tab) {
    ...
    case 'topWins': return [
      ...
      { key:'game',     label:'Jeu',          render: GameCell },
      { key:'bet',      label:'Mise',         render: Num,      align:'right' },
      { key:'payout',   label:'Gain (jetons)',render: GoldNum,  align:'right' },
      { key:'bet_id',   label:'Réf',          render: RefCell },
    ]
    case 'topBet': return [
      { key:'username', label:'Joueur',       render: PlayerCell },
      { key:'game',     label:'Jeu',          render: GameCell },
      { key:'bet',      label:'Mise (jetons)',render: GoldNum,  align:'right' },
    ...
    case 'mostPlayed': return [
      { key:'username',     label:'Joueur',        render: PlayerCell },
      { key:'games_played', label:'Parties jouées',render: GoldNum, align:'right' },
