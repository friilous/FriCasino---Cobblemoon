...
router.post('/login', async (req, res) => {
  try {
    ...
    res.json({ token, user: { id: user.id, username: user.username, is_admin: user.is_admin === 1, is_temp_pw: user.is_temp_pw === 1, balance: user.balance, total_wagered: user.total_wagered || 0, rank_id: user.rank_id || 1 } })
...
router.get('/me', authMiddleware, async (req, res) => {
  try {
    const result = await query(
      `SELECT id, username, is_admin, is_temp_pw, balance, created_at, total_wagered, rank_id FROM users WHERE id = $1`,
      [req.user.id]
    )
    const user = result.rows[0]
    if (!user) return res.status(404).json({ error: 'Utilisateur introuvable' })
    res.json({ ...user, is_admin: user.is_admin === 1, is_temp_pw: user.is_temp_pw === 1, total_wagered: user.total_wagered || 0 })
  } catch (err) { console.error(err); res.status(500).json({ error: 'Erreur serveur' }) }
})
