...
const MIN_BET = 10
// MAX_BET dynamique selon le rang — récupéré depuis le rank du joueur

async function getMaxBet(userId) {
  try {
    const { getRankFromWagered } = require('../utils/ranks')
    const r = await query(`SELECT total_wagered FROM users WHERE id = $1`, [userId])
    if (!r.rows[0]) return 10000
    return getRankFromWagered(r.rows[0].total_wagered).maxBet
  } catch { return 10000 }
}

function validateBet(bet, balance, maxBet = 10000) {
  if (!bet || isNaN(bet) || bet < MIN_BET) return `Mise minimum : ${MIN_BET} jetons`
  if (bet > maxBet) return `Mise maximum pour ton rang : ${maxBet.toLocaleString('fr-FR')} jetons`
...
// Met à jour total_wagered et vérifie montée de rang
async function updateWagered(userId, bet) {
  try {
    await query(
      `UPDATE users SET total_wagered = total_wagered + $1 WHERE id = $2`,
      [bet, userId]
    )
    if (global.updateRankIfNeeded) {
      await global.updateRankIfNeeded(userId)
    }
  } catch (err) {
    console.error('updateWagered error:', err.message)
  }
}
...
async function contributeToSuperJackpot(bet) {
  try {
    ...
    const contrib = Math.max(1, Math.floor(bet * pct))
    await query(
...
router.post('/slots', authMiddleware, async (req, res) => {
  try {
    ...
    const maxBet = await getMaxBet(req.user.id)
    const ur     = await query(`SELECT * FROM users WHERE id = $1`, [req.user.id])
    const user   = ur.rows[0]
    const err    = validateBet(bet, user.balance, maxBet)
    ...
    const hr = await query(
      `INSERT INTO game_history (user_id, game, bet, payout, meta) VALUES ($1,'slots',$2,$3,$4) RETURNING id`,
    ...
    await updateWagered(req.user.id, bet)
...
router.post('/plinko', authMiddleware, async (req, res) => {
  try {
    ...
    const hr = await query(
      `INSERT INTO game_history (user_id, game, bet, payout, meta) VALUES ($1,'plinko',$2,$3,$4) RETURNING id`,
...
router.post('/roulette', authMiddleware, async (req, res) => {
  try {
    ...
    const hr = await query(
      `INSERT INTO game_history (user_id, game, bet, payout, meta) VALUES ($1,'roulette',$2,$3,$4) RETURNING id`,
...
router.post('/blackjack', authMiddleware, async (req, res) => {
  try {
    ...
    if (action === 'deal') {
      ...
      await updateWagered(userId, bet)
      await contributeToSuperJackpot(bet)
      ...
      if (result.done) {
        ...
        const hr = await query(
          `INSERT INTO game_history (user_id, game, bet, payout, meta) VALUES ($1,'blackjack',$2,$3,$4) RETURNING id`,
    ...
    if (['hit', 'stand', 'double'].includes(action)) {
      ...
      if (action === 'double') {
        ...
        await updateWagered(userId, session.bet)
  ...
  try {
    ...
    if (action === 'reveal') {
      ...
      if (result.status === 'won') {
        ...
        await query(`UPDATE users SET balance = $1 WHERE id = $2`, [newBalance, userId])
        const hr = await query(
          `INSERT INTO game_history (user_id, game, bet, payout, meta) VALUES ($1,'mines',$2,$3,$4) RETURNING id`,
...
router.get('/live-feed', async (req, res) => {
  try {
    const result = await query(`SELECT * FROM live_feed ORDER BY created_at DESC LIMIT 50`)
...
router.get('/leaderboard', async (req, res) => {
  try {
    ...
    const VALID    = ['slots', 'plinko', 'roulette', 'blackjack', 'mines']
    const g        = VALID.includes(game) ? game : null

    const q1 = g
      ? `SELECT u.username, gh.game, gh.bet::int, gh.payout::int, CONCAT('#',UPPER(SUBSTRING(gh.game,1,3)),'-',LPAD(gh.id::text,6,'0')) AS bet_id FROM game_history gh JOIN users u ON u.id = gh.user_id WHERE gh.payout > gh.bet AND gh.game = $1 ORDER BY gh.payout DESC LIMIT 20`
      : `SELECT u.username, gh.game, gh.bet::int, gh.payout::int, CONCAT('#',UPPER(SUBSTRING(gh.game,1,3)),'-',LPAD(gh.id::text,6,'0')) AS bet_id FROM game_history gh JOIN users u ON u.id = gh.user_id WHERE gh.payout > gh.bet ORDER BY gh.payout DESC LIMIT 20`
    const topBestWin = await query(q1, g ? [g] : [])

    const q3 = g
      ? `SELECT u.username, gh.game, gh.bet::int, gh.payout::int, CONCAT('#',UPPER(SUBSTRING(gh.game,1,3)),'-',LPAD(gh.id::text,6,'0')) AS bet_id FROM game_history gh JOIN users u ON u.id = gh.user_id WHERE gh.game = $1 ORDER BY gh.bet DESC LIMIT 20`
      : `SELECT u.username, gh.game, gh.bet::int, gh.payout::int, CONCAT('#',UPPER(SUBSTRING(gh.game,1,3)),'-',LPAD(gh.id::text,6,'0')) AS bet_id FROM game_history gh JOIN users u ON u.id = gh.user_id ORDER BY gh.bet DESC LIMIT 20`
    const topBet = await query(q3, g ? [g] : [])

    const q4 = g
      ? `SELECT u.username, COUNT(gh.id)::int AS games_played, COALESCE(SUM(gh.bet),0)::int AS total_bet FROM users u JOIN game_history gh ON gh.user_id = u.id WHERE gh.game = $1 GROUP BY u.username ORDER BY games_played DESC LIMIT 20`
      : `SELECT u.username, COUNT(gh.id)::int AS games_played, COALESCE(SUM(gh.bet),0)::int AS total_bet FROM users u JOIN game_history gh ON gh.user_id = u.id GROUP BY u.username ORDER BY games_played DESC LIMIT 20`
    const mostPlayed = await query(q4, g ? [g] : [])

    res.json({
      topBestWin:  topBestWin.rows,
      topWins:     topBestWin.rows,
      topBet:      topBet.rows,
...
router.get('/player/:username', async (req, res) => {
  try {
    const ur = await query(
      `SELECT id, username, created_at, total_wagered, rank_id FROM users WHERE LOWER(username) = LOWER($1)`,
      [req.params.username]
    )
    if (!ur.rows[0]) return res.status(404).json({ error: 'Joueur introuvable' })
    const u = ur.rows[0]

    const [stats, byGame, bigWins] = await Promise.all([
      query(`SELECT COUNT(*)::int as games_played, COALESCE(SUM(bet),0)::int as total_bet, COALESCE(SUM(payout),0)::int as total_payout, COALESCE(MAX(payout-bet),0)::int as biggest_profit FROM game_history WHERE user_id = $1`, [u.id]),
      query(`SELECT game, COUNT(*)::int as plays, SUM(payout-bet)::int as profit, MAX(payout-bet)::int as best_win FROM game_history WHERE user_id = $1 GROUP BY game ORDER BY plays DESC`, [u.id]),
      query(`SELECT game, bet, payout, payout-bet as profit, created_at FROM game_history WHERE user_id = $1 AND payout > bet ORDER BY payout-bet DESC LIMIT 5`, [u.id]),
    ])
    ...
    res.json({
      ...
      stats: {
        ...
        net_profit:     s.total_payout - s.total_bet,
        biggest_profit: s.biggest_profit,
      },
      by_game:  byGame.rows,
      big_wins: bigWins.rows,
    })
...
router.get('/info', (req, res) => {
  res.json({
    slots:     { name: 'Slot Machine',     rtp: 88, minBet: MIN_BET },
    plinko:    { name: 'Plinko',           rtp: 91, minBet: MIN_BET },
    roulette:  { name: 'Roulette Pokémon', rtp: 89, minBet: MIN_BET },
    blackjack: { name: 'Blackjack',        rtp: 90, minBet: MIN_BET },
    mines:     { name: 'Mines',            rtp: 92, minBet: MIN_BET },
  })
})
