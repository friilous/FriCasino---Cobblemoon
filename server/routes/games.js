...
router.post('/crash', authMiddleware, async (req, res) => {
  try {
    ...
    await query(`UPDATE users SET balance = $1 WHERE id = $2`, [newBalance, user.id])
    const histResult = await query(
      `INSERT INTO game_history (user_id, game, bet, payout, meta) VALUES ($1, 'crash', $2, $3, $4) RETURNING id`,
  ...
  try {
    ...
    if (action === 'deal') {
      ...
      if (result.done) {
        ...
        const betId = generateBetId('blackjack', histResult.rows[0].id)
  ...
  try {
    ...
    if (action === 'reveal') {
      ...
      if (result.status === 'won') {
        ...
        const betId = generateBetId('mines', histResult.rows[0].id)
