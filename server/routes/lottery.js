...
async function initTables() {
  ...
  await db.run(`CREATE TABLE IF NOT EXISTS lottery_pot (
    id INTEGER PRIMARY KEY, amount INTEGER NOT NULL DEFAULT 0,
    draw_count INTEGER NOT NULL DEFAULT 1, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
  )`)
  await db.run(`CREATE TABLE IF NOT EXISTS lottery_tickets (
    id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL,
    username TEXT NOT NULL, draw_id INTEGER NOT NULL,
  ...
  await db.run(`CREATE TABLE IF NOT EXISTS lottery_history (
    ...
    winner TEXT NOT NULL, amount_won INTEGER NOT NULL,
    carried INTEGER NOT NULL, tickets INTEGER NOT NULL,
...
router.get('/', async (req, res) => {
  try {
    ...
    const tickets = await db.all(
      'SELECT username, COUNT(*) as count FROM lottery_tickets WHERE draw_id = ? GROUP BY username', [drawId]
    )
    const totalTickets = tickets.reduce((s, t) => s + t.count, 0)
    res.json({ pot: pot?.amount ?? 0, draw_id: drawId, ticket_price: TICKET_PRICE, total_tickets: totalTickets, players: tickets })
...
router.get('/mytickets', authMiddleware, async (req, res) => {
  try {
    ...
    const count  = await db.get(
      'SELECT COUNT(*) as n FROM lottery_tickets WHERE user_id = ? AND draw_id = ?', [req.user.id, drawId]
    )
...
router.post('/buy', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < qty; i++) {
      await db.run('INSERT INTO lottery_tickets (user_id, username, draw_id) VALUES (?,?,?)', [user.id, user.username, drawId])
    }
...
router.post('/draw', async (req, res) => {
  try {
    ...
    await db.run('UPDATE lottery_pot SET amount = ?, draw_count = draw_count + 1, updated_at = CURRENT_TIMESTAMP WHERE id = 1', [carried])
